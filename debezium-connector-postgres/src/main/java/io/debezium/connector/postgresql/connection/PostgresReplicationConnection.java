        ...
        try {
            ...
            execute(statement -> {
                ...
                try (ResultSet rs = statement.executeQuery(identifySystemStatement)) {
                    ...
                    //启动已有的replication slot
//                    try {
//                        pgConnection().getReplicationAPI().replicationStream().logical().withSlotName(slotName).start();
...
//                    }
    ...
    private ReplicationStream createReplicationStream(LogSequenceNumber lsn) throws SQLException {
        ...
        result.compareAndSet(0, 100865808);
        lsn = LogSequenceNumber.valueOf(result.get());
        final LogSequenceNumber totalLsn = LogSequenceNumber.valueOf(result.get());
        try {
